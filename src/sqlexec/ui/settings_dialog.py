...
class SettingsDialog(QDialog):
    ...
    def _create_general_tab(self):
        ...
        self.notification_check.setChecked(self.settings.general.enable_notifications)
        layout.addRow("启用通知:", self.notification_check)
        
        # 关闭动作
        self.close_action_combo = QComboBox()
        for action in CloseAction:
            self.close_action_combo.addItem(CloseAction.get_display_name(action), action)
        
        # 设置当前值
        index = self.close_action_combo.findData(self.settings.general.close_action)
        if index >= 0:
    ...
    def _create_groups_tab(self):
        ...
        remove_from_group_btn.clicked.connect(self._remove_connection_from_group)
    ...
    def _add_connection(self):
        ...
        if dialog.exec_():
            ...
            for i, (key, value) in enumerate(conn_info.items()):
                self.db_table.setItem(self.db_table.rowCount() - 1, i, QTableWidgetItem(str(value)))
            self.db_table.resizeColumnsToContents()
    ...
    def _test_connection(self):
        ...
        if success:
            success, error = self.main_window.db_manager.test_connection(conn.alias)
            if success:
    ...
    def accept(self):
        """保存设置"""
        # 更新常规设置
        self.settings.general.theme = Theme.LIGHT if self.theme_combo.currentText() == "浅色" else Theme.DARK
        self.settings.general.language = Language.ZH_CN if self.language_combo.currentText() == "简体中文" else Language.EN_US
        ...
        for row in range(self.group_table.rowCount()):
            ...
            if name:  # 只添加有名称的组
                ...
                connections = self.settings.groups.get(name, GroupInfo(name, "")).connections
                new_groups[name] = GroupInfo(name=name, description=description, connections=connections)
