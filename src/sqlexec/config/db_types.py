...
DB_TYPES: Dict[str, DbType] = {
    "mssql": DbType(
        ...
        name="SQL Server",
        parameters=[
            DbParameter("host", "主机地址", "text", True,
                        "localhost", "例如：localhost或127.0.0.1"),
            DbParameter("port", "端口", "number", False,
                        "1433", "SQL Server默认端口：1433"),
    ...
    "mysql": DbType(
        ...
        parameters=[
            DbParameter("host", "主机地址", "text", True,
                        "localhost", "例如：localhost或127.0.0.1"),
            DbParameter("port", "端口", "number", True,
                        "3306", "MySQL默认端口：3306"),
            DbParameter("database", "数据库名", "text", True, "", "要连接的数据库名"),
    ...
    "sqlite": DbType(
        ...
        name="SQLite",
        parameters=[
            DbParameter("database", "数据库文件路径", "file",
                        True, "", "选择或输入SQLite数据库文件路径"),
        ],
...
def build_connection_string(db_type: str, params: dict) -> str:
    ...
    # 移除空的可选参数
    clean_params = {k: v for k, v in params.items() if v or any(
        p.name == k and p.required for p in db_type_info.parameters)}
